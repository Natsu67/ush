...
static bool is_outer(int start, t_frmt_lst **arr) {
    ...
    for (p = arr[DOL_CMD]; p; p = p->next) if (start > p->data->start && start < p->data->end) return false;
...
static t_range *is_double_q(t_frmt_lst **arr, int start) {
    for (t_frmt_lst *p = arr[DBL_Q]; p; p = p->next) if (start == p->data->start) return p->data;
    return NULL;
}
...
void mx_create_outer_subst_n_dblq_list(char *s, t_frmt_lst **arr) {
    ...
    for (int i = 0; s[i]; i++) {
        ...
        else if (s[i] == M_SKP && (range = is_double_q(arr, i)) && is_outer(i, arr)) {
