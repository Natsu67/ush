...
static char *dollar_expansion(char **str, t_frmt_lst *list, t_ush *ush) {
    ...
    if ((*str)[list->data->start + 1] == '?') replace = mx_itoa(ush->last_return);
    else {
        buf = strndup(*str + list->data->start + 1, list->data->end - list->data->start);
        replace = (replace = getenv(buf)) ? strdup(replace) : calloc(1, 1);
        free(buf);
    }
...
static char *dollar_braces_expansion(char **str, t_frmt_lst *list, t_ush *ush) {
    ...
    if ((*str)[list->data->start + 2] == '?') replace = mx_itoa(ush->last_return);
    else {
        buf = strndup(*str + list->data->start + 2, list->data->end - list->data->start - 2);
        replace = (replace = getenv(buf)) ? strdup(replace) : calloc(1, 1);
        free(buf);
    }
...
char *mx_get_subst_replace_str(char **str, t_frmt_lst *list, t_ush *ush) {
    ...
    if ((*str)[list->data->start] == '`') {
        replace = strndup(*str + list->data->start + 1, list->data->end - list->data->start - 1);
        unslash_str(&replace);
    }
